...
__copyright__ = "HOPS 2020, API Manual for script 'psse.py'"
...
# <editor-fold desc="###  1. Power Flow Operation  ###">
...
# <editor-fold desc="###  4. Dynamics Simulation  ###">

# <editor-fold desc="##  4.1 Dynamic Simulation Function APIs  ##">
def dyre_new():
    print r"""
########################################################################################################################
...
def dynamics_solution_param_2():
    print r"""
########################################################################################################################

Use this API to modify the dynamic simulation solution parameters (except the channel output filename) in dynamics 
working memory (see PSS®E Program Operation Manual, activity ALTR).

Python syntax:
ierr = dynamics_solution_param_2(intgar, realar)

int[8] INTGAR - Is an array of eight elements (input; unchanged by default).
                INTGAR[0] network solution maximum number of iterations.
                INTGAR[1] number of output channels being monitored; must be less than or equal to the number of output
                            channels defined.
                INTGAR[2] number of state variables in use in the simulation model; should not normally be changed by 
                            the user.
                INTGAR[3] next available location in the CON array.
                INTGAR[4] next available location in the STATE array.
                INTGAR[5] next available location in the VAR array.
                INTGAR[6] next available location in the ICON and CHRICN arrays.
                INTGAR[7] next available location in the output channel arrays.

float[8] REALAR - Is an array of eight elements (input; unchanged by default).
                REALAR[0] acceleration sfactor used in the network solution.
                REALAR[1] convergence tolerance used in the network solution.
                REALAR[2] simulation time step (DELT).
                REALAR[3] filter time constant used in calculating bus frequency deviations.
                REALAR[4] intermediate simulation mode time step threshold used in extended term simulations.
                REALAR[5] large (island frequency) simulation mode time step threshold used in extended term 
                            simulations.
                REALAR[6] large (island frequency) simulation mode acceleration factor used in extended term 
                            simulations.
                REALAR[7] large (island frequency) simulation mode convergence tolerance used in extended term 
                            simulations.
...
# <editor-fold desc="##  4.13 Disturbance APIs  ##">
...
Use this API routine to apply a fault at a bus during dynamic simulations. 
(Note: use DIST_BUS_FAULT_2 if phase voltages are to be calculated during the simulation.)
...
int UNITS - Is the units in which fault admittance or impedance is specified in VALUES (input; 1 by default).
                ...
                UNITS = 2 admittance in mhos.
                UNITS = 3 impedance in ohms.
float BASEKV - Is the base voltage in kV used to calculate the per unit fault admittance if
                UNITS is 2 or 3; ignored if UNITS is 1. If BASEKV is specified as 0.0, the base voltage of bus IBUS is 
                        used (input; 0.0 by default).
float[2] VALUES - Is an array of two elements (input).
                    VALUES[0] is the real component of the complex fault admittance or impedance according to the value
                                specified for UNITS (0.0 by default).
                    VALUES[1] is the reactive component of the complex fault admittance or impedance according to the 
                                value specified for UNITS (-2.0E11 by default).
int IERR - Is the error code (output).
            IERR = 0 no error occurred
            IERR = 1 STRT or MSTR has not been successfully executed
            IERR = 2 bus not found
            ...
            IERR = 6 both BASEKV and the base voltage of bus IBUS are 0.0
...
int IBUS - Is the bus number of the bus to be disconnected (input; no default allowed).
int IERR - Is the error code (output).
            IERR = 0 no error occurred.
...
# <editor-fold desc="###  8. Subsystem Data Retrieval  ###">
...
int FLAG - Is a flag indicating which subsystem buses to include (input; 1 by default).
            FLAG = 1, for only in-service buses.
            FLAG = 2, for all buses.
int NSTR - Is the number of elements in STRING (1 < NSTR < 50 ) (input; no default allowed).
string[] STRING(NSTR) - Is an array of NSTR elements specifying NSTR of the following strings indicating the bus 
                        quantities desired (input; no default allowed):
                ...
                ’O_MISMATCH’ Bus mismatch, in units determined by the power output option setting 
...
int SID - Defines the bus subsystem to be used (input; -1 by default).
            ...
            SID = a valid bus subsystem identifier. Valid subsystem identifiers range from 0 to 11. Subsystem SID must 
                    have been previously defined.
int OWNER - Is a flag indicating owner usage if ownership is a subsystem selection criterion 
...
int FLAG - Is a flag indicating which subsystem branches to include (input; 1 by default).
            ...
            FLAG = 2 for all branches.
int NSTR - Is the number of elements in STRING (1 < NSTR < 50 ) (input; no default allowed).
string[] STRING(NSTR) - Is an array of NSTR elements specifying NSTR of the following strings indicating the branch 
                        quantities desired (input; no default allowed):
                ...
                ’PCTCORPRATE’ Percent from bus current or MVA loading (according to the appropriate percent loading 
                    units program option setting) of default rating set.
                ’PCTCORPRATEA’ Percent from bus current or MVA loading (according to the appropriate percent loading 
                    units program option setting) of rating set A.
                ’PCTCORPRATEB’ Percent from bus current or MVA loading (according to the appropriate percent loading 
                    units program option setting) of rating set B.
                ’PCTCORPRATEC’ Percent from bus current or MVA loading (according to the appropriate percent loading 
                    units program option setting) of rating set C.
            ...
            For the following, values are returned in MW, Mvar, or MVA:
                ...
                PSS®E 33.9 Subsystem Data Retrieval
            For the following, values are returned in units determined by the power output option setting:
...
int FLAG - Is a flag indicating which subsystem machines to include (input; 1 by default).
            ...
            FLAG = 4 for all machines.
int NSTR - Is the number of elements in STRING (1 < NSTR < 50 ) (input; no default allowed).
string[] STRING(NSTR) - Is an array of NSTR elements specifying NSTR of the following strings indicating the bus and/or 
                        machine quantities desired (input; no default allowed):
                ’FRACT1’ First owner fraction.
                ’FRACT2’ Second owner fraction.
                ’FRACT3’ Third owner fraction.
                ...
                ’WPF’ Power factor used in setting reactive power limits for the wind machine when its WMOD is 2 or 3; 
                        1.0 if the machine is not a wind machine.
...
int IERR - Is the error code (output):
            ...
            IERR = 5 DIM, and hence the size of RARRAY, is not large enough.
            IERR = 6 Invalid STRING value.
            IERR = 7 Sequence data not in case (when STRING = ’RPOS’, ’XSUBTR’, ’XTRANS’, or ’XSYNCH’).
...
# <editor-fold desc="###  12. Non-Engineering Functions  ###">
