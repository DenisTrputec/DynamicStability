...
def check_out_file(file_name):
    chars_to_remove = ['/', '\\', ':', '*', '?', '"', '<', '>', '|']
    file_name = file_name.translate(None, ''.join(chars_to_remove))
    if file_name == "":
...
def cli_options():
    ...
    while option == -1:
        ...
        if option == 1:
            ...
            # Add bus channels
            plot_counter = cli_add_bus_channels()
            # Initialize a PSSE dynamic simulation for state-space simulations and specify the Channel Output File
            psspy.strt(0, out_file)
            # Run option
...
def get_out_file():
    ...
    return check_out_file(file_name)
